...
type PacketSourceOption interface {
	Apply(opt *DecodeOptions)
}

type PacketSourceOptionFunc func(opt *DecodeOptions)

func (f PacketSourceOptionFunc) Apply(opt *DecodeOptions) {
	f(opt)
}

var (
	WithLazy = func(lazy bool) PacketSourceOption {
		return PacketSourceOptionFunc(func(opt *DecodeOptions) {
			opt.Lazy = lazy
		})
	}
	WithNoCopy = func(noCopy bool) PacketSourceOption {
		return PacketSourceOptionFunc(func(opt *DecodeOptions) {
			opt.NoCopy = noCopy
		})
	}
	WithPool = func(pool bool) PacketSourceOption {
		return PacketSourceOptionFunc(func(opt *DecodeOptions) {
			opt.Pool = pool
		})
	}
	WithSkipDecodeRecovery = func(skipDecodeRecovery bool) PacketSourceOption {
		return PacketSourceOptionFunc(func(opt *DecodeOptions) {
			opt.SkipDecodeRecovery = skipDecodeRecovery
		})
	}
	WithDecodeStreamsAsDatagrams = func(decodeStreamsAsDatagrams bool) PacketSourceOption {
		return PacketSourceOptionFunc(func(opt *DecodeOptions) {
			opt.DecodeStreamsAsDatagrams = decodeStreamsAsDatagrams
		})
	}
)
...
func NewZeroCopyPacketSource(source ZeroCopyPacketDataSource, decoder Decoder, opts ...PacketSourceOption) *PacketSource {
	...
	for idx := range opts {
		opts[idx].Apply(&src.DecodeOptions)
	}
