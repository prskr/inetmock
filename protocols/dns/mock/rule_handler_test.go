...
func TestRuleHandler_ServeDNS(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Resolve with fallback protocols",
			fields: fields{
...
func mockResponseWriter(tb testing.TB, expectedMsg interface{}) mdns.ResponseWriter {
	tb.Helper()
	ctrl := gomock.NewController(tb)
	resolver := mock2.NewRandomIPResolver(mustParseCIDR("10.10.0.0/8"))
